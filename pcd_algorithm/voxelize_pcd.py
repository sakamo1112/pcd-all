...
def remove_voxels(points: np.ndarray, colors: np.ndarray, ratio: float) -> o3d.geometry.VoxelGrid:
    """
    ...
    voxels_to_remove_indices = np.random.choice(len(points), num_voxels_to_remove, replace=False)
    ...
    new_voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(pcd_new, voxel_size=voxel_size)

    return new_voxel_grid


def voxelize_pcd(
    pcd: o3d.geometry.PointCloud, voxel_size: float, if_remove: bool=False, remove_ratio: float=0.9
    ...
    points = np.array([(voxel.grid_index * voxel_size + min_bound + voxel_size / 2) for voxel in voxel_grid.get_voxels()])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--if_remove", "-r", type=bool, default=False, help="True if remove voxels")
